...
export default function CompanyRegister() {
  ...
  const mockWallet = '0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045';
        ...
        <div className="glass-card animate-fade-up opacity-0-init delay-100" style={{ borderRadius: 16, padding: '2rem' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div>
              ...
              <input className="form-input" type="email" placeholder="admin@yourcompany.com" value={email} onChange={e => setEmail(e.target.value)} required />
            </div>
            <div>
            ...
            <button type="submit" className="btn-primary btn-md" style={{ marginTop: '0.25rem', justifyContent: 'center' }} disabled={loading}>
              {loading ? (
                <>
        ...
        <div className="animate-fade-up opacity-0-init delay-200" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginTop: '2rem' }}>
          {['Register', 'Select Vault', 'Dashboard'].map((s, i) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {i < 2 && <div style={{ width: 24, height: 1, background: 'rgba(90,112,144,0.3)', margin: '0 0.25rem' }} />}
            </div>
          ))}
        </div>
      </div>
